...
| `--bg-elevated` | `#0F0F11` | cards, dialogs, **artifact panel surface** (2026-08-02) |
...
The two halves of the terminal screen are DIFFERENT surface values, one step
apart in the same zinc ramp. No new hue, no tinted text, no status colour
involved.

| Half | Value | Source |
|---|---|---|
| Terminal side (pane tree, xterm) | `--bg-primary` `#0C0C0E` | terminal.ts, App.tsx root |
| Divider between them | 4px `--border` `#1E1E22` (drag: `#E4E4E766`) | ArtifactPanel.tsx PanelDivider |
| Panel left edge | 1px `--border-subtle` `#27272A`, both docked and overlay | ArtifactPanel.tsx:aside |
| Artifact panel (strip + header + body) | `--bg-elevated` `#0F0F11` | ArtifactPanel.tsx `PANEL_SURFACE` |
...
`transparent` so they take whichever host renders them: `#0F0F11` in the
...
| Panel tab strip | h 24px, bg `#0F0F11`, tab max-w 150px, 10.5px, active = `#151518` + inset 2px `--text-muted` + weight 600, `+` button 24px | ArtifactPanel.tsx TabStrip |
