...
│   ├── devServer.ts             → Dev-server URL detection (pure `detectDevServerHits` — ANSI-stripped PTY text → `{url, source}`, tagged from the banner — + `parseManualUrl`) and the per-session RANKED candidate store (`rankHits`: frontend > static > unknown > api; + the injected `setPreviewOpenCheck` that suppresses an offer for a URL already framed)
