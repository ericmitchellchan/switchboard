...
│   ├── ArtifactPicker.tsx       → The `+` picker: filterable KB docs + registry projects, repo files browsed one directory at a time (explorerList), the MANUAL URL row (type a port or a URL), and THIS SESSION'S DETECTED dev-server URLs ranked best-first with their source — where a candidate that lost the offer chip stays reachable
